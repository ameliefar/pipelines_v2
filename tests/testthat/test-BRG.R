...
test_that("format_BRG outputs all tables...", {
...
test_that("Brood_data returns an expected outcome...", {
  ...
  BRG_data <- dplyr::filter(pipeline_output$Brood_data, PopID %in% c("BRG"))
...
test_that("Capture_data returns an expected outcome...", {
  ...
  #Take a subset of only BRG data
  BRG_data <- dplyr::filter(pipeline_output$Capture_data, CapturePopID %in% c("BRG"))

  ## EK06579, banded as chick, recruited
  cap_EK06579 <- subset(BRG_data,
                        IndvID == "EK06579"
  ...
  expect_equal(cap_EK06579$ReleasePopID, "BRG")
  ...
  expect_equal(cap_HD86461$ReleasePopID, "BRG")
  ...
  expect_equal(cap_HD86485$ReleasePopID, "BRG")
...
test_that("Individual data returns an expected outcome...", {
  ...
  #Take a subset of only BRG data
  BRG_data <- dplyr::filter(pipeline_output$Individual_data, PopID %in% c("BRG"))

  #Individual EK06602, banded as chick, recruited
  ind_EK06602 <- subset(BRG_data, IndvID == "EK06602")
  expect_equal(ind_EK06602$Species, "PARMAJ")
  expect_equal(ind_EK06602$PopID, "BRG")
  ...
  ind_HH96002 <- subset(BRG_data, IndvID == "HH96002")
  expect_equal(ind_HH96002$Species, "CYACAE")
  expect_equal(ind_HH96002$PopID, "BRG")
  ...
  ind_HD86406 <- subset(BRG_data, IndvID == "HD86406")
  expect_equal(ind_HD86406$Species, "PERATE")
  expect_equal(ind_HD86406$PopID, "BRG")
...
test_that("Location_data returns an expected outcome...", {
  ...
  #Take a subset of only BRG data
  BRG_data <- dplyr::filter(pipeline_output$Location_data, PopID %in% c("BRG"))

  ## Nestbox 29
  loc_29 <- subset(BRG_data, LocationID == "29")
  expect_equal(loc_29$NestboxID, "29")
  expect_equal(loc_29$LocationType, "NB")
  expect_equal(loc_29$PopID, "BRG")
